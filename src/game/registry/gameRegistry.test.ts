...
describe('production game registry', () => {
  ...
  it('registers Vocal Hop as a single-player VOICE game without Pose or Hands', async () => {
    const vocalHop = gameRegistry.find(({ id }) => id === 'vocal-hop')
    expect(vocalHop).toMatchObject({
      id: 'vocal-hop',
      title: '聲控跳跳樂',
      category: 'VOICE',
      supportedTeams: [1],
