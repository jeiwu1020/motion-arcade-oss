...
export const BOWLING_POSE_INPUT_REQUEST = Object.freeze({
...
// oxlint-disable-next-line react/only-export-components
export async function startBowlingCamera(
  runtime: Pick<PoseGameplayInputRuntime, 'start'> | null,
): Promise<void> {
  await runtime?.start(BOWLING_POSE_INPUT_REQUEST)
}
...
function BowlingPoseSetupGuide() {
  return (
    <div className="bowling-pose-setup-guide" aria-label="上半身入鏡與投球提示">
      <strong>上半身完整入鏡</strong>
      <span>頭部、肩膀、手臂、手腕、軀幹與髖部都要看得見</span>
      <div>
        <span>左手或右手都可以投球</span>
        <span>看準瞄準標記再揮臂</span>
      </div>
    </div>
  )
...
export default function BowlingPoseGameScreen({ onExit }: { readonly onExit: () => void }) {
  ...
  const [session] = useState(() => new BowlingSession(sportsSource))
  ...
  const startCamera = useCallback(() => {
    void startBowlingCamera(runtimeRef.current).catch(() => undefined)
  }, [])
  const gamePhase = state.phase === 'FINISHED'
    ? 'RESULT'
    : state.phase === 'COUNTDOWN' ? 'COUNTDOWN' : 'PLAYING'
  const presentation = resolveCameraPresentation(poseSnapshot, gamePhase, 'UPPER_BODY')

  return (
    <main
      className="bowling-shell"
      data-input-mode="pose"
      data-presentation-mode={presentation.mode}
    >
      <BowlingTopbar state={state} onExit={onExit} />
      <CameraPresentationStage
        className="bowling-stage"
