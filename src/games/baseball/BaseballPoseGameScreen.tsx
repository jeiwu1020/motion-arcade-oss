...
export const BASEBALL_POSE_INPUT_REQUEST = Object.freeze({
...
// oxlint-disable-next-line react/only-export-components
export async function startBaseballCamera(
  runtime: Pick<PoseGameplayInputRuntime, 'start'> | null,
): Promise<void> {
  await runtime?.start(BASEBALL_POSE_INPUT_REQUEST)
}
...
function BaseballPoseSetupGuide() {
  return (
    <div className="baseball-pose-setup-guide" aria-label="上半身入鏡與揮棒提示">
      <strong>上半身完整入鏡</strong>
      <span>頭部、肩膀、手臂、手腕、軀幹與髖部都要看得見</span>
      <div>
        <span>左右手都能揮棒</span>
        <span>不需要拿球棒，請空手安全揮動</span>
      </div>
    </div>
  )
}
...
export default function BaseballPoseGameScreen({ onExit }: { readonly onExit: () => void }) {
  ...
  const [session] = useState(() => new BaseballSession(sportsSource))
  ...
  const startCamera = useCallback(() => {
    void startBaseballCamera(runtimeRef.current).catch(() => undefined)
  }, [])
  ...
  return (
    <main
      className="baseball-shell"
      data-input-mode="pose"
      data-presentation-mode={presentation.mode}
    >
      <BaseballTopbar state={state} onExit={onExit} />
      <CameraPresentationStage
        className="baseball-stage"
