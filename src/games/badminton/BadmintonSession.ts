...
import {
  ...
  type CreateBadmintonStateOptions,
} from './BadmintonCore'

export interface BadmintonTrackingInput {
  readonly setupReady: boolean
  readonly hardFailure: boolean
}

export interface BadmintonSessionOptions extends CreateBadmintonStateOptions {}

type Listener = () => void

/**
 * Bridges the immutable C0 Sports Motion snapshot into Badminton-local
...
export class BadmintonSession {
  ...
  constructor(source: SportsMotionSnapshotSource, options: BadmintonSessionOptions = {}) {
    this.#source = source
    this.#state = createBadmintonState(options)
  }

  readonly getState = (): BadmintonState => this.#state
  ...
  replay(): void {
    this.#state = replayBadminton(this.#state)
    this.#markCurrentSequences(this.#source.getSnapshot())
    this.#notify()
  }

  tick(deltaMs: number, tracking: BadmintonTrackingInput): void {
    ...
    this.#state = advanceBadminton(this.#state, {
      deltaMs: safeDeltaMs,
  ...
  #collectAttempts(attemptTimestampMs: number): readonly BadmintonSwingAttempt[] {
    const snapshot = this.#source.getSnapshot()
    const attempts: BadmintonSwingAttempt[] = []
  ...
  #collectEvent(
    ...
    attempts: BadmintonSwingAttempt[],
  ): void {
    ...
    attempts.push({
      hand,
      // The attempt clock is deterministic Badminton round time. C0's source
      // timestamp remains outside the game-local Core contract.
