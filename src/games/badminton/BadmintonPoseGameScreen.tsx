...
export const BADMINTON_POSE_INPUT_REQUEST = Object.freeze({
...
// oxlint-disable-next-line react/only-export-components
export async function startBadmintonCamera(
  runtime: Pick<PoseGameplayInputRuntime, 'start'> | null,
): Promise<void> {
  await runtime?.start(BADMINTON_POSE_INPUT_REQUEST)
}
...
function BadmintonPoseSetupGuide() {
  return (
    <div className="badminton-pose-setup-guide" aria-label="上半身入鏡與揮拍提示">
...
export default function BadmintonPoseGameScreen({ onExit }: { readonly onExit: () => void }) {
  ...
  const [session] = useState(() => new BadmintonSession(sportsSource))
  ...
  const startCamera = useCallback(() => {
    void startBadmintonCamera(runtimeRef.current).catch(() => undefined)
  }, [])
  ...
  return (
    <main
      className="badminton-shell"
      data-input-mode="pose"
      data-presentation-mode={presentation.mode}
    >
      <BadmintonTopbar state={state} onExit={onExit} />
      <CameraPresentationStage
        className="badminton-stage"
